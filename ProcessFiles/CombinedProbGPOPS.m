...
% mode 4: Cd Variation
% mode 5: Viscous Cd Variation - not implemented  - make this go from fully
% turbulent to fully laminar, get from alex in ratios of total drag
% mode 6: Max heating rate variation (?) - not implemented
% mode 7: Controllability limits (limited bank angle and aoa change rate) (?) - not implemented
% mode 8: Third Stage Mass Variation
% mode 9: Third Stage Thrust variation
% mode 10: SPARTAN mass  - not implemented
% mode 11: SPARTAN fuel mass  - not implemented
...
Isp_vars = [0.9 0.95 1 1.05 1.1] 
    for i = 1:length(Isp_vars)
        namelist{i} = ['Isp' num2str(Isp_vars(i)*100) '%'];
    end
...
Cd_vars = [0.9 0.95 1 1.05 1.1]  
    for i = 1:length(Cd_vars)
        namelist{i} = ['Cd' num2str(Cd_vars(i)*100) '%'];
    end

% Mode 5
elseif mode == 5
vCd_vars = [0.9 0.95 1 1.05 1.1] 
    for i = 1:length(vCd_vars)
        namelist{i} = ['vCd' num2str(vCd_vars(i)*100) '%'];
    end
    
% Mode 6
elseif mode == 6
heating_rate_vars = []  
    for i = 1:length(heating_rate_vars)
        namelist{i} = ['heating_rate' num2str(heating_rate_vars(i))];
    end

% Mode 7
elseif mode == 7
controllability_vars = []  
    for i = 1:length(controllability_vars)
        namelist{i} = ['controllability' num2str(controllability_vars(i))];
    end    
...
m3_vars = [0.9 0.95 1 1.05 1.1]  
    for i = 1:length(m3_vars)
        namelist{i} = ['m3' num2str(m3_vars(i)*100) '%'];
    end    
    
% Mode 9
elseif mode == 9
T3_vars = [0.9 0.95 1 1.05 1.1]  
    for i = 1:length(T3_vars)
        namelist{i} = ['T3' num2str(T3_vars(i)*100) '%'];
    end    
end
%% Misc Modifiers
...
elseif mode == 8 %
    for i = 1:length(m3_vars)  
        setup_variations{i} = setup;
        setup_variations{i}.bounds.phase(4).initialstate.lower = auxdata.Stage3.mTot*m3_vars(i);
        setup_variations{i}.bounds.phase(4).initialstate.upper = auxdata.Stage3.mTot*m3_vars(i);
        setup_variations{i}.bounds.phase(4).state.upper = auxdata.Stage3.mTot*m3_vars(i);
        setup_variations{i}.bounds.phase(4).finalstate.upper = auxdata.Stage3.mTot*m3_vars(i);
    end
elseif mode == 9 %
    for i = 1:length(T3_vars)  
        setup_variations{i} = setup;
        setup_variations{i}.auxdata.T3mod = T3_vars(i);
    end
end
...
Plotter(output,auxdata,auxdata.mode,auxdata.returnMode,namelist,M_englist,T_englist,engine_data,MList_EngineOn,AOAList_EngineOn,mRocket,mSpartan,mFuel,h0,v0,bounds);

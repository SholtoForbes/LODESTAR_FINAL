...
% mode 9: Third Stage Cd variation
...
Isp_vars = [0.9 0.95 1 1.05 1.1] 
    for i = 1:length(Isp_vars)
        namelist{i} = ['Isp' num2str(Isp_vars(i))];
    end
...
Cd_vars = [0.9 0.95 1 1.05 1.1]  
    for i = 1:length(Cd_vars)
        namelist{i} = ['Cd' num2str(Cd_vars(i))];
    end
...
vCd_vars = [0.9 0.95 1 1.05 1.1] 
    for i = 1:length(vCd_vars)
        namelist{i} = ['vCd' num2str(vCd_vars(i))];
    end
...
m3_vars = [0.9 0.95 1 1.05 1.1]  
    for i = 1:length(m3_vars)
        namelist{i} = ['m3' num2str(m3_vars(i))];
    end    
    
% Mode 9
elseif mode == 9
Cd3_vars = [0.9 0.95 1 1.05 1.1]  
    for i = 1:length(Cd3_vars)
        namelist{i} = ['Cd3' num2str(Cd3_vars(i))];
    end    
end
%% Misc Modifiers
...
elseif mode == 8 %
    for i = 1:length(m3_vars)  
        setup_variations{i} = setup;
        bounds.phase(4).initialstate.lower = auxdata.Stage3.mTot*m3_vars(i);
        bounds.phase(4).initialstate.upper = auxdata.Stage3.mTot*m3_vars(i);
    end
elseif mode == 9 %
    for i = 1:length(Cd3_vars)  
        setup_variations{i} = setup;
